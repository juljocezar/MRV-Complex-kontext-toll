...
export class ContradictionDetectorService {
    ...
    static async findContradictions(appState: AppState, newDocContext?: string): Promise<Contradiction[]> {
        ...
        const prompt = `
Du bist ein akribischer Analyst mit einem außergewöhnlichen Auge für Details. Deine einzige Aufgabe ist es, sachliche Widersprüche zwischen verschiedenen Informationen in der Akte zu finden. Sei extrem penibel. Suche auch nach subtilen oder impliziten Widersprüchen, die auf den ersten Blick nicht offensichtlich sind.
${newDocContext ? 
...
Wenn keine Widersprüche gefunden werden, gib ein leeres Array zurück.
        ...
        try {
            const results = await GeminiService.callAIWithSchema<Omit<Contradiction, 'id'>[]>(prompt, this.SCHEMA, appState.settings.ai, 'gemini-3-pro-preview');
